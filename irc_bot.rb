...
until s.eof? # until the server get to the end of file (EOF) - the ? denotes a boolean true or false.  
	...
	if msg.include? greeting_prefix && wasGreeted
		response = "BOO!"
		irc_server.puts 'PRIVMSG #{channel} :#{response}'  # output the response to the channel
	end
end
